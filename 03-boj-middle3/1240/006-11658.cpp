...
void makeFenwickTreeRow(int n, int arr[][LEN], int fenwickTree[][LEN], int i, int idx)
{
    int num=arr[i][idx];
    while(idx<=n)
    {
...
void updateFenwickTree(int n, int fenwickTree[][LEN], int arr[][LEN], int x, int y, int num)
{
    int xidx=x;
    while(xidx<=n)
    {   
        ...
        while(yidx<=n)
        {
            fenwickTree[xidx][yidx]-=arr[x][y];
            fenwickTree[xidx][yidx]+=num;
            yidx+=(yidx&-yidx);
        }
        xidx+=(xidx&-xidx);
    }
...
int main()
{
    ...
    for(int i=1;i<=n;i++)
    {
        ...
        {
            cin>>arr[i][j];
            makeFenwickTreeRow(n,arr,fenwickTree,i,j);
        }
    }
    for(int i=n;i>=1;i--)
    {
    ...
    for(int i=0;i<m;i++)
    {
        ...
        if(w==0)
        {
            int x,y,c;
            cin>>x>>y>>c;
            updateFenwickTree(n,fenwickTree,arr,x,y,c);
        }
        else if(w==1)
        {
            int x1,y1,x2,y2;
